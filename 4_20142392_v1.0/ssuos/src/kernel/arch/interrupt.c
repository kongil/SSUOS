...
void timer_handler(struct intr_frame *iframe)
{
	...
	/*change :  add if state is PROC_RUN and when cur_process->que_level is 2*/
	if ((cur_process -> pid != 0) && (!scheduling) && cur_process -> state == PROC_RUN) {
		cur_process->time_used++;	
		cur_process->time_slice++;	
		if(cur_process->que_level == 1 && cur_process->time_slice >= TIMER_MAX) {
			do_sched_on_return();
		}
		else if(cur_process->que_level == 2 && cur_process->time_slice >= 2 * TIMER_MAX) {
			do_sched_on_return();
		}
	}
