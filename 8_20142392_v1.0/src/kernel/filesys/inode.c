...
int inode_write(struct inode *in, uint32_t offset, char * buf, int len)
{
	...
	/*find error*/
	if(offset > in->sn_size || len <= 0 || buf == NULL)
		return -1;

	for (i = lbn_start; i <= lbn_end; i++)
	{
		blkoff = (offset + i * SSU_BLOCK_SIZE) / SSU_BLOCK_SIZE;  //block offset
		res_off = 0; 											  //remained after end
		if (i == lbn_end) {
			res_off = (offset + len) % SSU_BLOCK_SIZE;
			if (res_off == 0)
				res_off = SSU_BLOCK_SIZE;
		}
		/*for print*/
		if (i == lbn_start)
		...
		printk("end blkoff = %d\n", blkoff);
		if (i == lbn_start)
		printk("physically start blkoff = %d\n", lbn_to_pbn(in, blkoff));
		if (i == lbn_end)
		printk("physially end blkoff = %d\n", lbn_to_pbn(in, blkoff));

		/*read block in pbn*/
		memset(tmpblock, 0, SSU_BLOCK_SIZE);
		fs_readblock(fs, lbn_to_pbn(in, blkoff), tmpblock);

		/*if start block(lbn_start)*/
		if (i == lbn_start) {
			memcpy(tmpblock + offset, buf, SSU_BLOCK_SIZE - offset);
			buf += SSU_BLOCK_SIZE - offset;
		}
		/*if last block(lbn_end)*/
		else if (i == lbn_end) {
			//memcpy(tmpblock + res_off, buf, SSU_BLOCK_SIZE - res_off);
			//buf += SSU_BLOCK_SIZE - res_off;
			memcpy(tmpblock, buf, res_off);
			buf += res_off;
		}
		/*if middle block*/
		else {
		...
		/*rewrite to inode block*/
		fs_writeblock(fs, lbn_to_pbn(in, blkoff), tmpblock);
...
int inode_read(struct inode * in, uint32_t offset, char * buf, int len)
{
	...
	for (i = lbn_start; i <= lbn_end; i++) 
	{
		blkoff = (offset + i * SSU_BLOCK_SIZE) / SSU_BLOCK_SIZE;
		res_off = (offset + i * SSU_BLOCK_SIZE) % SSU_BLOCK_SIZE;

		/*if first block*/
		if (i == lbn_start) {
			if (len > SSU_BLOCK_SIZE)
				r_len = SSU_BLOCK_SIZE - offset % SSU_BLOCK_SIZE;
			else
				r_len = len;
		}
		/*if last block*/
			...
			res_off = 0;
			r_len = (offset + len) % SSU_BLOCK_SIZE;
		}
		/*if middle block*/
		else {
			res_off = 0;
			r_len = SSU_BLOCK_SIZE;
		}
		...
		/*read from tmpblock to buf*/
		memset(tmpblock, 0, SSU_BLOCK_SIZE);
		fs_readblock(fs, lbn_to_pbn(in, blkoff), tmpblock);

		memcpy(buf, tmpblock+res_off, r_len);
		buf += r_len;
	}
	return result;
	//
...
int lbn_to_pbn(struct inode * in, uint32_t lbn )
{
	...
	if (lbn < NUM_DIRECT) {
		/*if there is no indirectblock, make it*/
		if (in->sn_directblock[lbn] == 0) {
			balloc(fs->fs_blkmap, &(in->sn_directblock[lbn]));
			in->sn_nlink++;
			sync_bitmapblock(fs);
			memcpy(&tmpblock_indirect[in->cnt_data_block * 4], &in->cnt_data_block, 4);
		}

		pbn = in->sn_directblock[lbn];

	}
	else if (lbn >= NUM_DIRECT) {
		/*if there is no indirectblock, make it*/
		if (in->sn_indirectblock[in->cnt_data_block] == 0) {
			balloc(fs->fs_blkmap, &(in->sn_indirectblock[in->cnt_data_block]));
			in->sn_nlink++;
			sync_bitmapblock(fs);
		}

		/*read indirectblock's pbn*/
		long offset = (lbn - NUM_DIRECT) % 1024;

		/*read indirect block to tmpblock_indirect && read offset index of tmpblock_indirect*/
		memset(tmpblock_indirect, 0, SSU_BLOCK_SIZE);
		fs_readblock(fs, in->sn_indirectblock[in->cnt_data_block], tmpblock_indirect);
		memcpy(&pbn, tmpblock_indirect + offset, 4);

		/*if there is no directdata in indirect inode*/
		if (pbn == 0) {
		...
		/*rewrite to inode*/
		memcpy(tmpblock_indirect + offset, &pbn, 4);
		fs_writeblock(fs, in->sn_indirectblock[in->cnt_data_block], tmpblock_indirect);

		/*go to next indirect block*/
		if (offset == 1023) {
			in->cnt_data_block++;
		}
