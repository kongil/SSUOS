...
/*change : allocate another buffer each NULL_loginprompt, foreground, background*/
pid_t proc_create(proc_func func, struct proc_option *opt, void* aux) {
	...
	*(--top) = 6; //edi
	...
	if (opt == NULL) {							//first_login_prompt
	...
	else if (opt->foreground == TRUE) {			//foreground
		cur_foreground_process = p;
...
/*change : add for create_backgroudn*/
void create_shell_proc2(void* aux)
{
	struct proc_option proc_opt = {0, FALSE}; //prot_opt->foreground = FALSE;
	proc_create(login_prompt,&proc_opt,NULL);
}
...
/*add create_background to CmdList*/
void shell_proc(void* aux)
{
	CmdList cmdlist[] = {
		{"shutdown", 0, shutdown},
		...
		{"create_background", 0, create_shell_proc2},	 //added
	};
#define CMDNUM 5
#define TOKNUM 10
	char buf[BUFSIZ];
...
/*change : remover proc_create kernel1, 2*/
void idle(void* aux)
{
