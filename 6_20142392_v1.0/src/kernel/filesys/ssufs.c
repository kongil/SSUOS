...
struct vnode *make_vnode_tree(struct ssufs_superblock *sb, struct vnode *mnt_root)
{
	...
	/*recursive func for maping vnodetree from inode*/
	link_inodetree_to_vnodetree(mnt_root);
...
void link_inodetree_to_vnodetree(struct vnode *cur_vnode) {
	struct ssufs_inode *cur_inode;
	struct ssufs_inode *child_inode;
	struct vnode *child_vnode;
	struct dirent *dirent;
	int i, ndir = 0;

	cur_inode = (struct ssufs_inode *)(cur_vnode->info);
	ndir = num_direntry(cur_inode);

	/*for all the cur_inode, make child_vnode*/
	for (i = INODE_ROOT; i < ndir; i++) {
		ssufs_inode_read(cur_inode, sizeof(struct dirent) * i, (char*)dirent, sizeof(struct dirent));
		/*make child_inode*/
		child_inode = &ssufs_inode_table[dirent->d_ino];

		/*make child_vnode*/
		child_vnode = vnode_alloc();
		set_vnode(child_vnode, cur_vnode, child_inode);
	
		/*make curvnode's childlist*/
		list_push_back(&(cur_vnode->childlist), &(child_vnode->elem));
	
		/*recursive func*/
		link_inodetree_to_vnodetree(child_vnode);
	}
}
...
int ssufs_mkdir(char *dirname){
	...
	struct ssufs_inode *parent_new_inode;
	struct dirent dirent;

	new_inode = inode_alloc(DIR_TYPE);
	parent_new_inode = (struct ssufs_inode *)(cur_process->cwd->info); 

	dirent.d_ino = new_inode->i_no;
	dirent.d_type = SSU_DIR_TYPE;

	/*wirte "." directory entry to new_inode*/
	memcpy(dirent.d_name, ".", sizeof("."));
	ssufs_inode_write(new_inode, new_inode->i_size, (char *)&dirent, sizeof(struct dirent));

	/*wirte ".." directory entry to new_inode*/
	dirent.d_ino = parent_new_inode->i_no;
	memcpy(dirent.d_name, "..", sizeof(".."));
	ssufs_inode_write(new_inode, new_inode->i_size, (char *)&dirent, sizeof(struct dirent));

	/*wirte "dirname" directory entry to new_inode*/
	dirent.d_ino = new_inode->i_no;
	memcpy(dirent.d_name, dirname, strlen(dirname)+1);
	ssufs_inode_write(parent_new_inode, parent_new_inode->i_size, (char *)&dirent, sizeof(struct dirent));

	/*mappinf inode to vnode*/
	new_vnode = vnode_alloc();
	set_vnode(new_vnode, cur_process->cwd, new_inode);

	/*add new vnode to parent process's childlist*/
