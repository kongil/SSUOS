...
void child_stack_reset(pid_t pid){// (2)
	...
	memcpy(pda, pda, PAGE_SIZE);
	
	/*change : reset page directory of stack 0x40001000(va)*/
	pages = (void *)(VKERNEL_STACK_ADDR - PAGE_SIZE);	
   	pdi = pde_idx_addr(pages);
	pda[pdi] = 0;
	
	/*change : reset page directory of stack 0x40000000(va)*/
	pages = (void *)(VKERNEL_STACK_ADDR - 2 * PAGE_SIZE); 	
   	pdi = pde_idx_addr(pages);
	pda[pdi] = 0;
