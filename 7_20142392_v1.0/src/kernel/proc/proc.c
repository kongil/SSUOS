...
void lseek_proc(void *aux , void *filename)
{
	...
	/*   option  */
	if (strcmp(aux, "-a") == 0) {
		printk("cur pos : %d\n", lseek(fd, 2, SEEK_SET));		//set pos to 2

		printk("aft pos : %d\n", lseek(fd, 4, SEEK_CUR|A_OPT));//a optiom with 4step "ss0000uworld"

		lseek(fd, 0, SEEK_SET); 					//set pos to start
		read(fd, buf, 12);
		printk("%s\n", buf);
	}
	if (strcmp(aux, "-e") == 0) {
		printk("%d\n", lseek(fd, 3, SEEK_END|E_OPT)); //set pos to end + 3
	...
	if (strcmp(aux, "-re") == 0) {
		printk("%d\n", lseek(fd, -10, SEEK_SET|RE_OPT)); //set pos to start - 10
	...
	if (strcmp(aux, "-c") == 0) {
		...
		printk ("%d\n", lseek(fd, 3, SEEK_END|C_OPT));  //set po to end + 3 -> 3
		
		read(fd, result_buf, lseek(fd, 0, SEEK_CUR));	 //read file from SEEK_SET to SEEK_CUR							 
		printk("%s\n", result_buf);
	}
